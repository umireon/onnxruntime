...
class ConnectionGraph(object):
    def __init__(self, model):
        ...
        self.decompose_dispatcher = DecomposeDispatch()
        ...
        if name not in self.node_2_gnode:
            ...
            if isinstance(node, onnx.NodeProto):
                self.node_2_gnode[name].input_with_shapes = {
                    ...
                    and inp not in self.egraph.initializer_name2module
                    and (inp not in self.egraph.produced_by or self.egraph.produced_by[inp][0].op_type != "Constant")
                }
    ...
    def try_recompose(self, node: onnx.NodeProto, egraph: OnnxInGraph) -> (list):
    ...
    def recompose(self, egraph: OnnxInGraph):
        replace_nodes = {}
    ...
    def build_relationship(self):
        self.egraph = OnnxInGraph(self.model)
        ...
        for node in egraph.graph.node:
            ...
            for inp in node.input:
                if inp in egraph.produced_by:
                    ...
                    in_gnode = self.get_or_create_gnode(egraph.initializer_name2module[inp])
                    gnode.input_constant.append(in_gnode)
                elif "out_" + inp in egraph.graph_input_names:
                    in_gnode = self.get_or_create_gnode(egraph.node_name2module["out_" + inp])
            ...
            for out in node.output:
                if out in egraph.consumed_by:
                    ...
                    out_gnode = self.get_or_create_gnode(egraph.node_name2module["out_" + out], prefix="out_")
    ...
    for inp in node.current_node.input:
        ...
        if deco_name in buffer_cache:
            ...
            input_buffer.append(
                ComputeBuffer(
                    name=inp,
                    dtype=TENSOR_TYPE_TO_NP_TYPE[node.input_with_shapes[inp][0]],
            ...
            tv = (
                node.input_constant[node.input_constant.index(inp)].current_node
    ...
    for out in node.current_node.output:
        ...
        if deco_name in buffer_cache:
            ...
            output_buffer.append(
                ComputeBuffer(
                    name=out,
                    dtype=TENSOR_TYPE_TO_NP_TYPE[node.output_with_shapes[out][0]],
...
def lower_Node_to_IRNode(block: ExecutionBlock, graph_io: GraphIOBuffer, buffer_cache: Dict[str, ComputeBuffer]):
    group = block.group
    new_group = []
    for g in group:
...
class InterGroupStrategy(object):
    ...
    def do_fusion(self, nodes, loop_range: int):
        if not loop_range.is_number or loop_range <= 1024:
...
class ExecutionPrepare(object):
    def __init__(self, model):
        ...
        self.external_buffer: GraphIOBuffer = GraphIOBuffer()
    ...
    def extract_graph_io(self):
        ...
        for i in self.edge_graph.model.graph.input:
            ins = convert_onnx_value_to_computebuffer(i, prefix="out_")
            self.external_buffer.var_buffer_in.append(ins)
        for i in self.edge_graph.model.graph.output:
            outs = convert_onnx_value_to_computebuffer(i, prefix="out_")
