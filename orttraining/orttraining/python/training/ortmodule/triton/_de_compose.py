...
class DecomposeDispatch(object):
    ...
    def LayerNormalization(self, node: onnx.NodeProto, **kwargs):
        ...
        reducemean_out, reducemean_node = self.new_node("ReduceMean", [inputs[0]], f"{node.name}_reducemean", axes=[-1])
        ...
        add_out1, add_node1 = self.new_node("Add", [inputs[2], mul_out1], f"{node.name}_add", output=outputs)
    ...
    def ReduceMean(self, node: onnx.NodeProto, **kwargs):
        ...
        v = onnx.helper.make_tensor(
            name="last_dim", dims=(), data_type=onnx.TensorProto.FLOAT, vals=np.array([last_dim_v], dtype=np.float32)
        )
