...
from ._ir import ComputeBuffer, ExecutionBlock, IRNode, Loop, LoopAttr, MaskLoadNode, MaskStoreNode, PostProcessBlock
from ._sympy_utils import FloorDiv, sympy_symbol
...
class Schedule(object):
    ...
    def can_fusion_op(self, loop1: Loop, loop2: Loop):
        if loop1.var != loop2.var or loop1.start != loop2.start or loop1.end != loop2.end or loop1.step != loop2.step:
            return False
        return True

    @abstractmethod
    def do_fusion_recursive(self, loop1: Loop, loop2: Loop):
    ...
    def update_IO_after_fusion_op(
        ...
        bb1: ExecutionBlock,
        bb2: ExecutionBlock,
        global_input: Set[str],
        global_output: Set[str],
    ):
    ...
    @abstractmethod
    def fusion_op(self, blocks: List[ExecutionBlock], global_input: Set[str], global_output: Set[str]):
        if len(blocks) < 2:
            return blocks
        ...
        while len(de_blocks) > 1:
            bb1: ExecutionBlock = de_blocks.popleft()
            bb2: ExecutionBlock = de_blocks.popleft()

            if not isinstance(bb1.body, Loop):
                new_blocks.append(bb1)
                de_blocks.appendleft(bb2)
                continue
            if not isinstance(bb2.body, Loop):
                new_blocks.append(bb1)
    ...
    @abstractmethod
    def tile_inner_loop(self, blocks: List[ExecutionBlock], tile_size: int = 16) -> List[ExecutionBlock]:
        ...
        def do_tile_loop(loop: Loop):
            ...
            for sub_body in list_loop:
                if not isinstance(sub_body, Loop):
                    mutate_body.append(sub_body)
                    continue
                assert sub_body.depth == 0, "only support tile loop with depth 0"

                main_loop: Loop = sub_body

                main_loop_range = FloorDiv(main_loop.end - main_loop.start, sympy_factor)
                ...
                mutate_body.append(PostProcessBlock(pre_loop))
    ...
    @abstractmethod
    def enable_recompute_for_reduction(self, blocks: List[ExecutionBlock]):
        return blocks

    @abstractmethod
    def vectoring_inner_loop(self, blocks: List[ExecutionBlock], lanes: int = 16) -> List[ExecutionBlock]:
        assert len(blocks) == 1, " only support one block now, but got {} blocks".format(len(blocks))
        bb1: ExecutionBlock = blocks[0]
        if not isinstance(bb1.body, Loop):
            return blocks

        def do_vectoring_loop(loop: Loop):
            if loop.depth > 0:
                if isinstance(loop.body, list):
            ...
            if (
                ...
                loop.attributes = LoopAttr.ScalarLoop
                assert loop.body[0].vectorization == False, "op in this loop should not be vectorized"
                return loop
            loop.vectorization = True
            loop.step = lanes
            for op in loop.body:
                assert isinstance(op, IRNode), f"expected op to be IRNode, but got {type(op)}"
    ...
    @abstractmethod
    def parallelize_outer_loop(self, blocks: List[ExecutionBlock], parallel_depth: int = 2) -> List[ExecutionBlock]:
        ...
        assert len(blocks) == 1, " only support one block now, but got {} blocks".format(len(blocks))
        bb = blocks[0]
        assert isinstance(bb.body, Loop), "only support parallelize outer loop"
        ...
        if bb.body.depth < parallel_depth:
            return blocks
        assert bb.body.depth + 1 > parallel_depth, "parallel depth should be smaller than loop depth"
        assert bb.body.start == 0 and bb.body.step == 1, "only support parallelize natural nest loop"

        bb.body.parallel = True

        if parallel_depth == 2:
            assert isinstance(bb.body.body, Loop), "only support parallelize outer loop"
...
class GPUSchedule(Schedule):
    ...
    def tile_inner_loop(self, blocks: List[ExecutionBlock], tile_size: int = 16) -> List[ExecutionBlock]:
        ...
        def do_tile_loop(loop: Loop):
            ...
            for sub_body in list_loop:
                ...
                main_loop: Loop = sub_body
                mutate_body.append(main_loop)
                mutate_body.append(PostProcessBlock(main_loop))
                mutate_body[-1].global_connections = bb1.connections
            loop.body = mutate_body
            return loop
    ...
    @staticmethod
    def get_all_inputs_need_recompute(
        loop: Loop,
    ...
    @staticmethod
    def collect_predecessors_for_recompute(
        inputs: List[ComputeBuffer],
        graph_inputs: List[ComputeBuffer],
        graph_outputs: List[ComputeBuffer],
        load_map: Dict,
    ...
    def enable_recompute_for_reduction(self, blocks: List[ExecutionBlock]):
        ...
        def do_recompute(top_loop: Loop):
            if top_loop.depth > 1:
            ...
            list_loop: List[Loop] = top_loop.body if isinstance(top_loop.body, list) else [top_loop.body]
            if len(list_loop) == 1:
                return top_loop
    ...
    def vectoring_inner_loop(self, blocks: List[ExecutionBlock], lanes: int = 1024) -> List[ExecutionBlock]:
        assert len(blocks) == 1, " only support one block now, but got {} blocks".format(len(blocks))
        bb1: ExecutionBlock = blocks[0]
        if not isinstance(bb1.body, Loop):
            return blocks
        ...
        def do_vectoring_loop(loop: Loop):
            ...
            try_simplify = (loop.end - loop.start) > lanes
            if loop.start != 0 or loop.step != 1 or loop.vectorization or loop.parallel:
                loop.attributes = LoopAttr.ScalarLoop
                assert loop.body[0].vectorization == False, "op in this loop should not be vectorized"
                return loop
            loop.vectorization = True
            if try_simplify.is_Boolean and try_simplify:
                loop.step = sympy_symbol(name="RBLOCK")
            else:
                loop.step = loop.end
            for op in loop.body:
                assert isinstance(op, IRNode), f"expected op to be IRNode, but got {type(op)}"
    ...
    def parallelize_outer_loop(self, blocks: List[ExecutionBlock]) -> List[ExecutionBlock]:
        assert len(blocks) == 1, " only support one block now, but got {} blocks".format(len(blocks))
        bb = blocks[0]
        assert isinstance(bb.body, Loop), "only support parallelize outer loop"
        ...
        if parallel_depth >= 2:
            this_loop: Loop = bb.body
            nest_loops = []
            while this_loop.depth > 1:
                assert isinstance(this_loop.body, Loop), "only support parallelize outer loop"
                assert (
